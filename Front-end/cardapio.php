...
<!--===============================================================================================-->
	<link rel="stylesheet" type="text/css" href="css/cardapio/util.css">
	<link rel="stylesheet" type="text/css" href="css/cardapio/main.css">
      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-center justify-content-center">
          <div class="col-md-9 ftco-animate text-center">
            <h1 class="mb-2 bread">Cardápio Escolar</h1>
          </div>
        </div>
      </div>
    </section>
	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">	
							...
							$controle_botao = 0;
							$result_cafe = "SELECT * FROM cafe_intervalo ORDER BY id_intervalo";
							$resultado_cafe = mysqli_query($conn, $result_cafe);
							while ($row_cafe = mysqli_fetch_assoc($resultado_cafe)){ 
								if($controle_botao <> 1){?>
								<br>
								<div class="item active">
										<table id="tablePreview" class="table table-bordered">
											  ...
											  <tbody>
												<tr>
												  <th scope="row">Intervalo da manhã</th>
												  <td><?php echo utf8_encode($row_cafe['segunda'])?></p></td>
												  <td><?php echo utf8_encode($row_cafe['terca'])?></p></td>
												  <td><?php echo utf8_encode($row_cafe['quarta'])?></p></td>
												  <td><?php echo utf8_encode($row_cafe['quinta'])?></p></td>
												  <td><?php echo utf8_encode($row_cafe['sexta'])?></p></td>
												</tr>
											</div>	
														<?php
													}
										...
										?>
												...
												$controle_botao = 0;
												$result_almoco = "SELECT * FROM cardapio_almoco ORDER BY id_almoco";
												$resultado_almoco = mysqli_query($conn, $result_almoco);
												while ($row_almoco = mysqli_fetch_assoc($resultado_almoco)){ 
														if($controle_botao <> 1){?>
															<br>
															<div class="item active">
												<tr>
												  <th scope="row">Almoço</th>
												  <td><?php echo utf8_encode($row_almoco['segunda'])?></p></td>
												  <td><?php echo utf8_encode($row_almoco['terca'])?></p></td>
												  <td><?php echo utf8_encode($row_almoco['quarta'])?></p></td>
												  <td><?php echo utf8_encode($row_almoco['quinta'])?></p></td>
												  <td><?php echo utf8_encode($row_almoco['sexta'])?></p></td>
												</tr>
												</div>
														<?php
													}
											...
											?>
												<?php
												$controle_botao = 0;
												$result_janta = "SELECT * FROM cardapio_janta ORDER BY id_janta";
												$resultado_janta = mysqli_query($conn, $result_janta);
												while ($row_janta = mysqli_fetch_assoc($resultado_janta)){ 
														if($controle_botao <> 1){?>
															<br>
															<div class="item active">
												<tr>
												  <th scope="row">Janta</th>
												  <td><?php echo utf8_encode($row_janta['segunda'])?></p></td>
												  <td><?php echo utf8_encode($row_janta['terca'])?></p></td>
												  <td><?php echo utf8_encode($row_janta['quarta'])?></p></td>
												  <td><?php echo utf8_encode($row_janta['quinta'])?></p></td>
												  <td><?php echo utf8_encode($row_janta['sexta'])?></p></td>
												</tr>
												</div>	
												<?php
											}
